...
class Text : public Component {
   private:
    std::vector<ColoredChar> content; // Stores colored characters. Data is held in one line.
    std::vector<size_t> lineBreaks; // Indexes of the start of each line in content. This is used for width and height printing.
	std::variant<std::string, std::u32string> rawText; // Original text.

   public:
    Text();
    Text(uint32_t xCoord, uint32_t yCoord, const std::variant<std::string, std::u32string>& textContent,
...
inline Text::Text(uint32_t xCoord, uint32_t yCoord,
                  const std::variant<std::string, std::u32string>& textContent, uint8_t r, uint8_t g,
                  uint8_t b)
    : Component(xCoord, yCoord) {
		rawText = textContent;
    uint32_t rgba = (static_cast<uint32_t>(r) << 24) | (static_cast<uint32_t>(g) << 16) | (static_cast<uint32_t>(b) << 8) | 0xFF;
    std::visit([&](auto const &str) {
        for (auto ch : str) {
