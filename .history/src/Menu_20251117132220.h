...
* 
* This class is the big container for the music player. It handles rendering the
* components that make up each menu screen.
...
#include "ColoredChar/ColoredChar_Helpers.h"
#include "ColoredChar/ColoredChar_Constants.h"
...
/**
 * Represents a menu object. Holds components and handles rendering. 
 */
class Menu {
   private:
...
inline void Menu::render() {
    ...
    std::cout << " there are " << components.size() << " components to render\n";
    for (const auto& comp : components) {
        std::cout << "Rendering component at (" << comp->getX() << ", " << comp->getY() << ") with size (" << comp->getWidth() << "x" << comp->getHeight() << ")\n";
        for (uint32_t y = 0; y < comp->getHeight(); ++y) {
            
            for (uint32_t x = 0; x < comp->getWidth(); ++x) {
                int bufX = comp->getX() + x + 1; // We don't want anything on the border
    ...
    for (const auto& row : renderBuffer) {
        for (const auto& pixel : row) {
            ...
            std::cout << pixel.ansiColor() << toUTF8(pixel.c)
                      << ColoredChar::ansiReset();
        }
        std::cout << '\n';
    }
}

inline void Menu::addComponent(Component* c) {
