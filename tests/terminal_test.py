...
try:
    ...
    assert rows[6][41:54] == 'test-tone.mp3', 'title moved'
    assert rows[7][41:51] == 'Local file', 'artist row moved'
    assert rows[14][49:51] == '<<' and rows[14][63:65] == '>>', 'controls moved'
    assert rows[4][6] == '█', 'album art moved'
    assert b'Local file' in out and b'<<' in out and b'>>' in out, 'original controls missing'
    assert '█'.encode() in out, 'original album art missing'
    os.write(master, b' ')
    out += read_for(0.3)
